...
class Lifting(BaseEstimator):
    ...
    random_seed: int
    grid_bounds: Tuple[Tuple[float, float], Tuple[float, float]] = ((0, 1), (0, 1))
    grid: np.ndarray = None
    weights: np.ndarray = None
    data_mean: np.ndarray = None 
    data_std: np.ndarray = None

    def _append_grid(self, x): #?remove reshape, [None,...]
        expanded_grid = np.repeat(self.grid[None,...], len(x), axis=0)
        return np.stack([x, expanded_grid], axis=-1)

    def fit(self, x, y=None):
        if self.grid is None: #?change grid_bounds to tuple of tuples ((0,1), (0,1))
            x_grid = np.linspace(*self.grid_bounds[0], x.shape[-2])
            y_grid = np.linspace(*self.grid_bounds[1], x.shape[-1])
            self.grid = np.meshgrid(x_grid, y_grid, indexing="ij")
            self.grid = np.stack(self.grid, axis=-1)[...,0]
    ...
    def transform(self, x):
        ...
        return lifted.transpose(0, 3, 2, 1)


@dataclass       
class FourierBlock(Dense, Linear):
    n_hidden_channels: int = None
    n_modes: Tuple[int, int] = None
    avoid_complex: bool = True
    ...
    def transform(self, x, y=None):
        ...
        fft_x = fft_transform.transform(x)
        for channel, pipeline in enumerate(self.block_pipelines):
                fft_x[:, channel] = pipeline.transform(fft_x[:, channel]).reshape(fft_x[:, channel].shape)
...
class FNO2D(BaseEstimator):
    ...
    def transform(self, x):
        ...
        if self.n_blocks > 0:
            lifted_x = self.fourier_pipeline.transform(lifted_x)
        lifted_x = lifted_x.reshape(lifted_x.shape[0], -1)
        return self.projection_pipeline.transform(lifted_x).reshape(x.shape)
